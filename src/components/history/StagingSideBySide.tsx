...
export function StagingSideBySide() {
    ...
    <div className="flex h-full w-full">
      ...
      <div className="flex flex-1 flex-col border-r border-border">
        <div className="flex h-8 shrink-0 items-center justify-center border-b border-border bg-muted text-xs font-semibold text-muted-foreground">
          {currentVersion.label} — Committed
        </div>
      ...
      {/* After (working state with diff annotations) */}
      <div className="flex flex-1 flex-col">
        ...
        <div className="flex-1">
          <ReactFlowProvider>
            <SyncedPane
              id="after"
              nodes={state.stagingNodes}
              edges={[...state.stagingEdges, ...state.stagingGhostEdges]}
              nodeTypes={nodeTypes}
              edgeTypes={edgeTypes}
              sourceRef={sourceRef}
            />
